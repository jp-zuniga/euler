...
inline int64_t mult_and_compare(const uint64_t &big_prod, const uint64_t &a,
                                ...
                                const uint64_t &d) {
  uint64_t new_prod = a * b * c * d;
  return std::max(new_prod, big_prod);
}

inline uint64_t find_biggest(const std::vector<std::vector<uint8_t>> &grid,
  ...
  // go through the rows from right to left
  for (size_t i = 0; i < grid_size; i++) {
    for (size_t j = 0; j <= grid_size - 4; j++) {
      ...
      c = grid[i][j + 2];
      d = grid[i][j + 3];
      big_prod = mult_and_compare(big_prod, a, b, c, d);
    }
  }

  // the rest of the loops have the same logic,
  // only difference being the ranges they iterate over
  // and the selected indices for multiplication

  // since multiplication is commutative,
  // the vertical/horizontal direction of the loops is irrelevant

  // go through the columns from top to bottom
  ...
  // iterate diagonally down-right, right to left, top to bottom
  for (size_t i = 0; i <= grid_size - 4; i++) {
    for (size_t j = 0; j <= grid_size - 4; j++) {
      a = grid[i][j];
      b = grid[i + 1][j + 1];
  ...
  // iterate diagonally down-left, left to right, top to bottom
  for (size_t i = 0; i <= grid_size - 4; i++) {
    for (size_t j = 3; j < grid_size; j++) {
      a = grid[i][j];
      b = grid[i + 1][j - 1];
      c = grid[i + 2][j - 2];
      d = grid[i + 3][j - 3];
