...
inline void solve_p8() {
  ...
  uint64_t biggest_prod = 0;
  uint64_t current_prod = 1;

  // .size() - 12 to avoid going out of bounds in the nested loop
  for (size_t i = 0; i < big_num.size() - 12; i++) {
    current_prod = 1;

    // multiply big_num[i] and the following 12 digits together
    for (size_t j = i; j <= i + 12; j++) {
      current_prod *= big_num[j];
    }
